...
void UPxiiDebugTraceBPLibrary::DrawDebugArrowSimple(const UObject* WorldContextObject, FVector Start, FVector End, FLinearColor Color, float Thickness, float Duration)
{
	if (!WorldContextObject) return;
	UWorld* World = WorldContextObject->GetWorld();
	if (!World) return;
	DrawDebugDirectionalArrow(World, Start, End, 10.f, Color.ToFColor(true), false, Duration, 0, Thickness);
}